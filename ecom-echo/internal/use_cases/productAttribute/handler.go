...
func (h *Handler) Create(c echo.Context) error {
	...
	return c.JSON(http.StatusCreated, 
			response.Success("Atributo de producto creado exitosamente", productAttribute))
}
...
func (h *Handler) GetByID(c echo.Context) error {
	...
	return c.JSON(http.StatusOK, 
			response.Success("Atributo de producto obtenido exitosamente", productoAttribute))
}
...
func (h *Handler) Update(c echo.Context) error {
	...
	var req UpdateProductAttributeRequest
	if err := c.Bind(&req); err != nil {
			return c.JSON(http.StatusBadRequest, 
					response.Error("Error al procesar la solicitud (posible error de sintaxis)", err.Error()))
	}
	...
	productAttribute, err := h.service.Update(c.Request().Context(), id, &req)
	if err != nil {
			return h.handleError(c, err)
	}

	return c.JSON(http.StatusOK, 
			response.Success("Atributo de producto actualizado exitosamente", productAttribute))
}
